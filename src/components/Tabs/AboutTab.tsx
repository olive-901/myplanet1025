...
const schoolLinks: Record<string, string> = {
  "서울중곡초등학교 졸업": "https://cg.sen.es.kr/",
  "용마중학교 졸업": "https://yongma.sen.ms.kr/",
  "해성여자고등학교 졸업": "https://haesung-g.sen.hs.kr/",
  "한림대학교 영어영문학과 재학": "https://www.hallym.ac.kr/hallym/index.do"
};
      ...
      <div className="lg:col-span-5 space-y-8">
        ...
        >
          ...
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            {/* Contact Email info */}
            <a
              href={`mailto:${SUMIN_PROFILE.email}`}
              className="text-xs text-slate-400 font-mono flex items-center justify-center gap-1 bg-[#100f21] hover:bg-[#15132d] hover:border-purple-500/40 border border-slate-800 w-full px-3.5 py-2 rounded-full transition-all group shadow-inner cursor-pointer"
            >
              <span className="text-cyan-400 group-hover:text-cyan-300">EMAIL:</span>
              <span className="text-slate-300 font-bold group-hover:text-white underline decoration-cyan-500/30">{SUMIN_PROFILE.email}</span>
            </a>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                ...
                <a
                  href="https://youtu.be/UOZvdWN5OeA?si=8QW1mbKHeAcoUeoD"
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 py-1 px-3 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/25 text-cyan-200 border border-cyan-500/20 text-[10px] font-bold transition hover:scale-[1.03] active:scale-95 shadow-sm group/btn"
                >
                  <span className="truncate">바로가기</span>
                  <ExternalLink className="w-2.5 h-2.5 text-cyan-400 group-hover/btn:translate-x-0.5 transition-transform" />
                </a>
              </div>
          ...
          <div className="mt-4 bg-purple-950/20 border border-purple-500/5 p-3 rounded-2xl h-14 flex items-center justify-center text-center overflow-hidden">
            <p className="text-[11px] text-purple-200/80 animate-pulse italic leading-relaxed">
              {isPlaying ? lyrics[Math.min(lyrics.length - 1, Math.floor((playerProgress / 100) * lyrics.length))] : "Click Play to hear Pluto's space loop..."}
            </p>
          </div>
            ...
            {SUMIN_PROFILE.education.map((edu, idx) => (
              <div key={idx} className="flex gap-4 group/edu relative">
                ...
                <div className="flex-1 pb-3">
                  ...
                  {schoolLinks[edu.school] ? (
                    <div className="mt-0.5">
                      <a
                        href={schoolLinks[edu.school]}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-200 hover:text-purple-300 transition group/link underline decoration-dotted decoration-purple-500/40 hover:decoration-solid underline-offset-4 cursor-pointer"
                      >
                        <span>{edu.school}</span>
                        <ExternalLink className="w-3.5 h-3.5 text-slate-500 group-hover/link:text-purple-300 group-hover/link:translate-x-0.5 transition-all" />
                      </a>
                    </div>
                  ) : (
